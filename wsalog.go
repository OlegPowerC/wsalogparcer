...
func main() {
	...
	for scan1.Scan() {
		...
		if allargs[5] == cmdg || allargs[5] == cmdpo || allargs[5] == cmdpu || allargs[5] == cmdd || allargs[5] == cmdc {
			if len(*user) > 0 {
				if strings.Contains(strings.ToLower(allargs[7]), strings.ToLower(*user)) == false {
					continue
				}
			}
			if len(*ip) > 1 {
			...
			if datetimefilterflagst {
				if uits.Unix() < unixformatst {
					continue
				}
			}

			if datetimefilterflagen {
				if uits.Unix() > unixformatet {
					continue
				}
			}
